...
def get_emails(days: int, from_email: str, to_email: str) -> str:
    ...
    EMAIL_ADDRESS = os.getenv('GOOGLE_ADDRESS')
    PASSWORD = os.getenv('GOOGLE_APP_PASSWORD')

    if not all([IMAP_SERVER, EMAIL_ADDRESS, PASSWORD]):
        print("エラー: 環境変数 IMAP_SERVER, GOOGLE_ADDRESS, GOOGLE_APP_PASSWORD を設定してください。")
        return ""

    output_path = os.path.join(os.path.dirname(__file__), '..', '..', 'output', 'mail_report.txt')
    all_email_bodies = []
    ...
    try:
        ...
        if not email_ids:
            ...
            mail.logout()
            # メールがなくてもエラーではないので、空のファイルを生成して正常終了する
            ...
            if status == 'OK':
                for response_part in msg_data:
                    if isinstance(response_part, tuple):
                        ...
                        # text/plain が見つからなければ、text/html を探す
                        if not body:
                            if msg.is_multipart():
                                for part in msg.walk():
    ...
    if not all_email_bodies:
        print("警告: メール本文を解析・抽出できませんでした。")
        # 本文がなくてもエラーではないので、空のファイルを生成して正常終了する
        open(output_path, 'w').close()
        return output_path
...
if __name__ == '__main__':
    ...
    DAYS_TO_FETCH = 7
    FROM_EMAIL = "mailmag@mag2premium.com"
